...
func (g *Game) HandleResults() bool {
	...
	if g.resultStep >= 4 && inpututil.IsKeyJustPressed(ebiten.KeySpace) { //Condition de relance du jeu
		g.resultStep = 0
		<-g.cResultat
		return true
	}
...
func (g *Game) Update() error {
	...
	case StateChooseRunner:
		...
		case <-g.cEcritureClient:
			for i := 0; i < 4; i++ {
				g.runners[i].colorScheme = g.runnersColors[i]
			}
			g.UpdateAnimation()
			g.state++
		default:
