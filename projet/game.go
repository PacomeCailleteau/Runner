...
type Game struct {
	...
	runnersColors   [4]int        //Colors des joueurs
	cEcritureClient chan bool
	cTemp           chan bool
	cResultat       chan string
	cNbJoueurCo     chan int
	cOk             chan bool
	crejoue         chan int //indique le nombre de joueurs qui rejouent
}
...
func InitGame() (g Game) {
	g.cEcritureClient = make(chan bool, 1)
	g.cTemp = make(chan bool, 1)
	g.cResultat = make(chan string, 1)
	g.cNbJoueurCo = make(chan int, 1)
	g.crejoue = make(chan int, 1)
	g.cOk = make(chan bool, 1)
